...
#[tokio::test]
async fn test_process_signaler_with_execute() {
  ...
  // Subscribe before executing to ensure we receive the notification
  let _receiver = signaler.subscribe();

  // Run the execution in a local set
  let local_set = tokio::task::LocalSet::new();
  let exit_code = local_set.run_until(execute_future).await;

  assert_eq!(exit_code, 0);

  // We should have received a PID notification for the echo command
  // (echo is a builtin, so no external process is spawned)
  // The test validates that the mechanism works without errors
}
...
async fn test_process_signaler_tracks_external_command() {
  ...
  // Note: We use /usr/bin/sleep directly because `sleep` is a builtin in deno_task_shell
  let list = parse("/usr/bin/sleep 0.1").unwrap();
  let kill_signal = KillSignal::default();
  ...
  let seen_pid = Rc::new(Cell::new(None::<u32>));
  ...
  assert_eq!(exit_code, 0);
  assert!(
    seen_pid.get().is_some(),
    "Should have seen a PID during execution of the sleep command, got: {:?}",
    seen_pid.get()
  );
}

#[tokio::test]
async fn test_process_signaler_current_pid_after_execution() {
  use std::collections::HashMap;

  // Use an external command that takes some time
  let list = parse("/usr/bin/sleep 0.01").unwrap();
  let kill_signal = KillSignal::default();
  ...
  // After execution, current_pid should be None
